...
const TeamCalendar = ({
    ...
    useEffect(() => {
        teamMatchesRequested();
        API.getTeamMatches(id).then((res) => teamMatchesLoaded(res.data));
    ...
    if (matches.length) {
        if (filteredMatches.length) {
            content = filteredMatches.map((calendarItem) => (
                <CalendarItem calendarItem={calendarItem} key={calendarItem.id} />
            ));
        } else {
            content = (
                <div>
        ...
        content = (
            <div>
                <img className="calendarList-errorImg" src={refereeYellow} alt="" />
                <div className="calendarList-error">Не найдено ни одного матча</div>
                <Link to="/teams" className="calendarList-errorBack">
